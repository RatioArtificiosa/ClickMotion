...
export function Loader({ onReveal, assetsReady = true }: Props) {
  ...
  useEffect(() => {
    if (done) return;
    ((window as unknown as { __msLenis?: { stop: () => void; start: () => void } }).__msLenis)?.stop();
    ...
    const id = window.setInterval(() => ((window as unknown as { __msLenis?: { stop: () => void; start: () => void } }).__msLenis)?.stop(), 150);
    return () => {
      ...
      ((window as unknown as { __msLenis?: { stop: () => void; start: () => void } }).__msLenis)?.start();
    };
  }, [done]);
