...
const HUD_GAUGE_AT = 10;
const HUD_RING_SEC = 2.8;

type ScrollNarrativeApi = {
  setProgress: (p: number) => void;
  setHudClock?: (seconds: number) => void;
  getProgress: () => number;
  getTarget: () => number;
  pageOwns: () => boolean;
  productId: string;
...
export default function SkySpiresHero() {
  ...
  const captureHudRef = useRef(false);
  ...
  const setHudClock = useCallback((seconds: number) => {
    captureHudRef.current = true;
    const t = Number.isFinite(seconds) ? Math.max(0, seconds) : 0;
    setGauge(t >= HUD_GAUGE_AT);
    setRing(Math.floor(t / HUD_RING_SEC) % GAUGE.rings.length);
  }, []);
  ...
  useEffect(() => {
    const boot = window.setTimeout(() => {
      if (!captureHudRef.current) setGauge(true);
    }, 10000);
    const rings = window.setInterval(() => {
      if (captureHudRef.current) return;
