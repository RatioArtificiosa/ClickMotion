import Lenis from 'lenis';
  ...
  const ASSET = '/assets/zero-energy';
  const canLabels = [ASSET + '/textures/zero-energy_texture_double-litchi.webp', ASSET + '/textures/zero-energy_texture_coco-citron-vert.webp', ASSET + '/textures/zero-energy_texture_Kiwi-Concombre.webp', ASSET + '/textures/zero-energy_texture_peche-blanche.webp', ASSET + '/textures/zero-energy_texture_pomme-rhubarbe.webp', ASSET + '/textures/zero-energy_texture_abricot_framboise.webp'];
  ...
  const mainEl =
    document.querySelector('main.main-wrapper') || document.querySelector('main');
  if (!mainEl) {
    throw new Error('Zero Energy: missing <main> mount');
  }
  ...
  hdriLoader.load(ASSET + '/webgl/hdri2.hdr', function (texture) {
  ...
  spot3.map = new THREE.TextureLoader().load(ASSET + '/img/spot-mask.avif');
  ...
  const baseGltf = await gltfLoader.loadAsync(ASSET + '/webgl/base.glb');
  ...
  const canGltf = await gltfLoader.loadAsync(ASSET + '/webgl/can.glb');

  const canMetallic = new THREE.TextureLoader().load(ASSET + '/img/can-metallic-2.avif');
  ...
  // Dedicated WebGL context-loss flag, distinct from animation.paused.
  ...
  window.addEventListener(
    'wheel',
    (e) => {
      if (reducedMotion || !ENABLE_DESKTOP_PAGING || pointer.prevent) return;
  ...
  const dispose = () => {
    ...
    window.carousel = undefined;
    window.loader = undefined;
    document.body.style.overflow = '';
    document.body.style.cursor = '';
    document.body.classList.remove('is-hud-ready', 'is-profile-active');
    window.__zeDispose = undefined;
  };
  window.__zeDispose = dispose;
  return dispose;
}

export async function initWebglScene() {
  if (typeof window !== 'undefined' && window.__zeDispose) {
    window.__zeDispose();
  }
  return bootWebglScene();
}

export function disposeZeroEnergyWebgl() {
  if (typeof window !== 'undefined' && window.__zeDispose) {
    window.__zeDispose();
  }
}