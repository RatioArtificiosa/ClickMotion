...
You are about to place a **product line reveal** on your site: visitors meet each SKU one by one on a quiet bone stage, with a living 3D vessel and editorial copy.

**No Scroller (pin-until-complete).**

- Scroll aims the lineup. The page does not physically scroll while the viewing runs.
- Each product lands with a 3D cross-fade, bloom, and copy card.
- Wheel, trackpad, finger, and keys advance the journey. Snap settles on a SKU.
- When the last product ends, the pin releases so the rest of your page may continue.
...
| **CUSTOMIZATION.md** | Later: swap products, count, mesh, colors. |
...
```
Build LINEUP using only the files in this pack folder.
Read PROMPT.md and follow it exactly. Use the source and assets provided.
No Scroller: pin-until-complete. One 100dvh stage. Scroll aims on N viewports (N = PRODUCTS.length).
Snap on lift. Release at 0 plus up or 1 plus down.
...
Keep gsap for SKU cross-fade tweens only. Do not use ScrollTrigger to pin.
```

6. Confirm:
   - Full-viewport bone stage
   - The **page does not physically scroll** during the viewing
   - Each SKU lands with 3D vessel + copy
   - After the last product, one more down-scroll lets the page continue
...
```tsx
import LineupSection from "./source/LineupSection";
...
**Do not** set `overflow: hidden` on the page. After the lineup finishes, the next section must be able to scroll in.
...
> Fix this for me without asking me to write code. Keep No Scroller: pin-until-complete. Scroll aims on N viewports. Snap on lift. After release at the end, the page owns until dock. Never restore ScrollTrigger pin, lenis, SmoothScroll, or a tall multi-vh sticky track. Do not add PSAVE. Do not ask me to write code.
