 ...
 * When expanding PRODUCTS, either:
 *  - add keys here ("04": "/textures/labels/…"), OR
 *  - set product.labelPath and pass labelUrl into Can3D (preferred).
 */
...
type Props = {
  skuNumber?: string;
  /** Prefer this over LABEL_MAP when set (from product.labelPath). */
  labelUrl?: string;
  /** Optional GLB path override (default /models/can.glb). */
  meshUrl?: string;
...
export function Can3D({
  ...
  const labelTex = useMemo(() => {
    ...
    const url =
      labelUrl || LABEL_MAP[skuNumber] || LABEL_MAP["01"] || "/textures/labels/still-01-clear-2.png";
