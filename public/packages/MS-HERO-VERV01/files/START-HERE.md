...
| **CUSTOMIZATION.md** | Later: rebrand lockup, marquee, colors, film, CTAs. |
| **VIDEO_GEN_PROMPT.md** | Optional: generate a new culture film (pure world, no UI). |
| **assets/** | Client film + poster only (no storefront previews). |
| **source/** | Production React hero (drop-in). |

Default names, tokens, and proof numbers are a **starting board**. Rebuild them until every pixel reads as your brand alone.
