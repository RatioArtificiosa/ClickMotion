/**
 * Upsert MS-HERO-MERI01 from content/prompts/heroes/MS-HERO-MERI01.mdx into CMS store.
 * Keeps sortOrder/likes from prior row. Pin-until-complete gold body must match MDX.
 */
const fs = require("fs");
...
const raw = fs.readFileSync(
  path.join(root, "content/prompts/heroes/MS-HERO-MERI01.mdx"),
  "utf8"
);
...
if (!/pin-until-complete/i.test(body) && !/3\.2/.test(body)) {
  throw new Error("MDX body missing pin-until-complete / 3.2 gold markers");
}
if (/ScrollTrigger\.create/.test(body) && /420vh/.test(body) && !/Hard ban/.test(body)) {
  // allow ban language that mentions 420vh; fail only pure legacy ST.create as primary
}
...
const product = {
  ...
  previewVideoFullscreen: "/assets/videos/meridian-scroll-preview-fs-v1.mp4",
  ...
  estimatedTokens: Number(get("estimatedTokens") || 7200),
  ...
  dependencies: [{ name: "gsap", version: "^3.12.0", required: true }],
