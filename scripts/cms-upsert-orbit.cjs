...
const raw = fs.readFileSync(
  path.join(root, "content/prompts/heroes/MS-HERO-ORBI01.mdx"),
  "utf8"
);
...
const product = {
  ...
  estimatedTokens: Number(get("estimatedTokens") || 13800),
...
if (idx >= 0) {
  ...
  store.products[idx] = {
    ...
    likes: prev.likes ?? 380,
  };
} else {
  store.products.push({ ...product, likes: 380 });
}
