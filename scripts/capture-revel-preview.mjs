/**
 * Burn Revel scroll-narrative UI into a gallery/product preview.
 * Drives pin-until-complete via window.__msScrollNarrative.setProgress (no tall-page scroll).
 * Live wheel gold (do not recapture unless asked): gain 0.11, swipe 0.09, lag 0.55.
 *
 * Usage:
 *   node scripts/capture-revel-preview.mjs
 *   node scripts/capture-revel-preview.mjs http://127.0.0.1:3004/demo/cleanroom-revel
 */
...
const URL =
  process.argv[2] || "http://127.0.0.1:3004/demo/cleanroom-revel";
const OUT_DIR = path.join(root, "tmp", "revel-preview-frames");
const OUT_VIDEO = path.join(
  root,
  "public",
  "assets",
  "videos",
  "revel-scroll-preview-v1.mp4"
);
const OUT_VIDEO_FS = path.join(
  ...
  "assets",
  "videos",
  "revel-scroll-preview-fs-v1.mp4"
);
const OUT_POSTER = path.join(
  ...
  "assets",
  "posters",
  "revel-scroll-preview-v1.webp"
);
const OUT_THUMB = path.join(root, "public", "thumbnails", "MS-HERO-REVL01.webp");
const FPS = 24;
const DURATION_S = 20;
...
async function main() {
  ...
  const posterFrame = path.join(OUT_DIR, "frame-0048.png");
  const midFrame = path.join(OUT_DIR, "frame-0240.png");
