/**
 * Isolated Playwright visual pass for Nori, Sol, Quill, Brine, Helm.
 * Fresh Chromium. Does not attach to the operator's Chrome.
 */
...
const out = path.join(__dirname, "visual-pass-wow-group-3");
fs.mkdirSync(out, { recursive: true });

const SKUS = [
  { id: "nori", sel: ".nori-root", quote: ".nori-quote" },
  { id: "sol", sel: ".sol-root", quote: ".sol-headline" },
  { id: "quill", sel: ".quill-root", quote: ".quill-name" },
  { id: "brine", sel: ".brine-root", quote: ".brine-name" },
  { id: "helm", sel: ".helm-root", quote: ".helm-question" },
];
...
(async () => {
  // Use installed Chrome. The npm package is present; bundled Chromium is a separate 190MB download.
  ...
  for (const sku of SKUS) {
    ...
    if (sku.id !== "sol") {
      ...
      await page.click(".sol-cta");
      await page.waitForTimeout(200);
      const sent = await page.evaluate(() => document.querySelector(".sol-cta")?.innerText);
      console.log("SOL-SENT", sent);
      await page.screenshot({ path: path.join(out, "sol-desktop-sent.png"), fullPage: false });
    }
  }
