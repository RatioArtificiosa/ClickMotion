/**
 * Isolated Playwright visual pass for Vesper, Slip, Reed, Dune, Bolt.
...
const out = path.join(__dirname, "visual-pass-wow-group-5");
fs.mkdirSync(out, { recursive: true });

const SKUS = [
  { id: "vesper", sel: ".vesper-root", quote: ".vesper-name", pin: true },
  { id: "slip", sel: ".slip-root", quote: ".slip-question", pin: true },
  { id: "reed", sel: ".reed-root", quote: ".reed-headline", pin: false, click: ".reed-cta" },
  { id: "dune", sel: ".dune-root", quote: ".dune-name", pin: true },
  { id: "bolt", sel: ".bolt-root", quote: ".bolt-figure", pin: true },
];
  ...
  return page.evaluate(
    ({ s, extra }) => {
      ...
      return {
        ...
        status: document.querySelector("h1, h2") ? 200 : 0,
      };
    },
...
(async () => {
  ...
  for (const sku of SKUS) {
    const res = await page.goto(`${BASE}/demo/cleanroom-${sku.id}`, {
      waitUntil: "domcontentloaded",
      timeout: 45000,
    });
    console.log(sku.id.toUpperCase() + "-STATUS", res && res.status());
