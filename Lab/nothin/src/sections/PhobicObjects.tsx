...
import { PHOBIC_ITEMS } from "../data/phobic";

/**
 * #phobic-objects — black void, formes that evade the pointer.
 *
 * Port of live noth.in `Kv()` (nothinv1.netlify.app/main.js → `.formes-w`):
 *
 * - CSS rest pose; GSAP x/y are offsets from that rest.
 * - Distance is mouse → **rest center** (not the live position).
 * - If mouse inside influenceRadius of rest → radial flee (power4.out 0.45s).
 * - Else → elastic home x:0 y:0 (1.2s).
 *
 * Idle vs pointer-in (matches live screenshots):
 * - **No mouse in window:** treat pointer as viewport **center** → objects are
 *   pushed outward from center → spread composition.
 * - **Mouse enters** (even at the edge): real coords; if far from rest, objects
 *   elastic back to the tight home cluster, then flee when approached.
 *
 * Lab: /lab/phobic-objects
 */
export function PhobicObjects() {
  ...
  useEffect(() => {
    ...
    const objects = Array.from(
      layer.querySelectorAll<HTMLElement>("[data-phobic]"),
    );

    // Cache base rotation (live: data-formes-base-rot)
    for (const el of objects) {
      if (el.dataset.baseRot === undefined) {
        const fromItem = PHOBIC_ITEMS.find((i) => i.id === el.dataset.phobic);
    ...
    /** Rest-center in viewport coords (live o()) — subtract current GSAP offset */
    ...
    /** Desktop / mobile params — exact live values */
    const params = () =>
    ...
    /** Live: when pointer is outside, fake mouse = viewport center (spread). */
    const centerPointer = () => ({
    ...
    const onLeave = () => {
      ...
      // Live mouseleave: re-apply with viewport center → objects spread outward
    ...
    // Initial state: no pointer → spread from center (live idle look)
  ...
  return (
    <section
      ...
      style={{ cursor: "none" }}
    >
      {/* formes stage — absolute rest poses, GSAP offsets on top */}
      <div
        ref={layerRef}
        className="formes-w absolute left-1/2 top-1/2 h-[min(100%,1056px)] w-[min(100%,1872px)] -translate-x-1/2 -translate-y-1/2"
      >
        {PHOBIC_ITEMS.map((item) => {
          if (item.letter) {
            return (
              <span
                key={item.id}
                data-phobic={item.id}
                className="pointer-events-none absolute select-none font-display font-medium text-white will-change-transform"
                style={{
                  left: `${item.left}%`,
                  top: `${item.top}%`,
